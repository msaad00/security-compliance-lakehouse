...
| Connector catalog            | Static connector access-boundary catalog plus CLI validation/listing and UI/API state for configure, probe, and run history.                                                                                                   | `connectors/catalog.json`, `src/security_lakehouse/connectors.py`, `tests/test_connector_state.py`           |
| Framework catalog and badges | Source-linked framework registry, readiness gates, reviewed mappings, crosswalks, and TrustOps-owned SVG badges. Badges are not certification seals.                                                                           | `frameworks/registry.json`, `mappings/control_map.json`, `docs/images/frameworks/`, `tests/test_mappings.py` |
...
| Enterprise connector ingestion    | Direct production ingestion for every source system in the catalog.                                          | The catalog documents access boundaries. Only local fixtures, catalog validation, connector state, probes, and lake-shaped outputs are shipped. |
| Full framework coverage           | Exhaustive official-control evaluation for every listed framework.                                           | Frameworks are source-linked and readiness-gated with limited mappings. Badges communicate support state, not certification.                    |
...
Artifact: connector access-boundary JSON and framework readiness rows.
...
| Framework badge strip                                         | Available at `docs/images/trustops-framework-badges.svg`.                                                                                                        |
| Individual framework badges                                   | Available under `docs/images/frameworks/`.                                                                                                                       |
